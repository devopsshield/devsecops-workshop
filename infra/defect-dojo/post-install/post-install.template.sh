...
export instanceName="__INSTANCE_NAME__"

# set all variables here:
export USERNAME="__USERNAME__"
export PASSWORD="__PASSWORD__"
export postgresHostname="$instanceName-postgresql.__DOMAIN__"
export NEWHOSTNAME="$instanceName.__DOMAIN__"
export nginx_folder="$HOME/__REPO_NAME__/nginx"
export databaseName="defectdojo"
export databasePort="5432"
export doPauses="__DO_PAUSES__" # set to "true" to pause after each step
# wait time for the container to be up and migrations to be done
export waitTime=300

export EMAIL="__EMAIL__"
export adminUser="__ADMIN_USER__"
export adminPassword="__ADMIN_PASSWORD__"
...
find $HOME/__REPO_NAME__/docker/environments/postgres-redis.env -type f -exec sed -i "s/__DD_DATABASE_USER__/$USERNAME/g" {} \;
find $HOME/__REPO_NAME__/docker/environments/postgres-redis.env -type f -exec sed -i "s/__DD_DATABASE_PASSWORD__/$PASSWORD/g" {} \;
find $HOME/__REPO_NAME__/docker/environments/postgres-redis.env -type f -exec sed -i "s/__DD_DATABASE_HOST__/$postgresHostname/g" {} \;
find $HOME/__REPO_NAME__/docker/environments/postgres-redis.env -type f -exec sed -i "s/__DD_DATABASE_NAME__/$databaseName/g" {} \;
find $HOME/__REPO_NAME__/docker/environments/postgres-redis.env -type f -exec sed -i "s/__DD_DATABASE_PORT__/$databasePort/g" {} \;

# show the files
cat $HOME/__REPO_NAME__/docker/environments/postgres-redis.env
...
sudo cp $HOME/__REPO_NAME__/systemd/defectdojo-composer.service /etc/systemd/system/
